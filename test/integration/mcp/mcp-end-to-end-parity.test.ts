...
/**
 * Copy the shared fixtures tree into a fresh temp directory and
 * return its absolute path. Excludes `_build` (stdlib cache, per-
 * fixture and regenerated by the first Agda load anyway) and any
 * stray `.agda-mcp-tmp-*` files left by a prior failed atomic
 * write.
 *
 * Each `withHarness` call now runs against its own copy so that
 * proof-action tools with `writeToFile: true` (the default after
 * this branch landed) can't contaminate fixtures for later tests
 * in the same run. Without this isolation, an `agda_give` in one
 * test would persist its edit to the shared fixture file and
 * every subsequent test loading the same file would observe the
 * modified version — exactly the regression that first surfaced
 * in CI after the write-to-file default shipped.
 */
function isolateFixtures(): string {
  const dir = mkdtempSync(join(tmpdir(), "agda-mcp-fixtures-"));
  cpSync(FIXTURES, dir, {
    recursive: true,
    filter: (src) => {
...
async function withHarness(run: (harness: any) => Promise<void>, projectRoot?: string) {
  // Default: hand each test an ephemeral copy of the fixtures dir so
  // write-to-file-by-default proof actions can't pollute the shared
  // fixture sources for subsequent tests. Explicit projectRoot
  // overrides (e.g. tests that need to probe a specific layout) are
  // passed through untouched and the caller owns their lifecycle.
  const ownsTempRoot = projectRoot === undefined;
  const effectiveRoot = ownsTempRoot ? isolateFixtures() : projectRoot;
